...
# A collection of helper classes and functions
# ----------------------------------------------
...
def remove_mask_and_warp(src, pos, padding_mask, level_start_index, spatial_shapes):
    """ Removes padding mask in sequence and warps彎曲 each level of tokens into fixed-sized sequences.
...
class DomainAttention(nn.Module):
    ...
    def __init__(self, d_model, n_heads, dropout, as_gen=False):
        super(DomainAttention, self).__init__()
        self.grl = nn.Identity() if as_gen else GradientReversal()
    ...
    def forward(self, query, src, pos=None, padding_mask=None):
        ...
        r_query, _ = self.cross_attn(
            query=query.transpose(0, 1), #[1, 4, 256]

...
class METRO_Encoder(BertPreTrainedModel):
    ...
    def __init__(self, config, args):
        ...
        #domain query dimension reduction layer
        self.query_dim_red=nn.Linear(self.img_dim, self.config.hidden_size, bias=True)
    ...
    def forward(self, img_feats, joint_query, vertex_query, is_train, input_ids=None, token_type_ids=None, attention_mask=None,
        ...
        if is_train == True and self.args.da_mode == 'uda' and self.config.output_feature_dim!=3:
...
class METRO(BertPreTrainedModel): #有三層METRO 也就是三層encoder block
    ...
    def __init__(self, config, args):
        ...
        #domain query dimension reduction layer  
        self.query_dim_red = nn.Linear(config.hidden_size, self.config.output_feature_dim)      
    ...
    def forward(self, input, input_ids=None, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        #domain query 到最後一層不降維
        if is_train == True and self.args.da_mode == 'uda' and self.config.output_feature_dim!=3 and self.config.output_feature_dim!=128:
            if self.args.joint_align:
...
class METRO_Body_Network(torch.nn.Module):
    ...
    def __init__(self, args, config, backbone, trans_encoder, mesh_sampler):
        ...
        if self.joint_align:
            self.joint_D = MLP(256, 256, 1, 3) #input size=128, hidden=128, output size=1, 3層
    ...
    def forward(self, images, smpl, mesh_sampler, meta_masks=None, is_train=False):
        ...
        image_feat = self.backbone(images)                              #[4, 2048, 7, 7]
        # print("img feat: ",image_feat.size())
        image_feat_newview = image_feat.view(batch_size,2048,-1)
        image_feat_newview = image_feat_newview.transpose(1,2)          #[4, 49, 2048]
        # print("feature size: ", image_feat_newview.size())
        ...
        if is_train==True:
            ...
            features = features*meta_masks + constant_tensor*(1-meta_masks)            
            if self.joint_align:
                joint_query = self.joint_query.expand(batch_size, -1, -1)   #(batch_size, 1, 2051)
            if self.vertex_align:
        ...
        if is_train and self.da_mode == 'uda':
            ...
            #only output source domain predictions
            pred_3d_joints = pred_3d_joints[:batch_size//2, :, :]
            pred_vertices_sub2 = pred_vertices_sub2[:batch_size//2, :, :]

...
wandb.init(project="METRO-UDA", entity="kelly-pei2",config={"batch size": 4})
...
def run(args, train_dataloader, val_dataloader, METRO_model, smpl, mesh_sampler, renderer):
    ...
    for iteration, (img_keys, images, annotations) in enumerate(train_dataloader): #train_dataloader會將dataset分成好幾小份
        ...
        #mask for target domain: all 1, 不遮蔽
        if args.da_mode=='uda':
            annotations['mjm_mask'] = torch.cat((annotations['mjm_mask'], torch.ones_like(annotations['mjm_mask'])), dim=0)
            annotations['mvm_mask'] = torch.cat((annotations['mvm_mask'], torch.ones_like(annotations['mvm_mask'])), dim=0)
        ...
        if iteration % iters_per_epoch == 0 or iteration == max_iter:
            ...
            # writer.add_scalar("joint domain loss", log_loss_joint_domain.avg, iteration / iters_per_epoch)
            # writer.add_scalar("vertex domain loss", log_loss_vertex_domain.avg, iteration / iters_per_epoch)
            ...
            wandb.log({"loss_2d_joints": log_loss_2djoints.avg,
                        ...
                        "loss_backbone_domain": log_loss_backbone_domain.avg,
...
def main(args):
    ...
    # os.environ['WORLD_SIZE']: number of GPU

...
def run(args, train_dataloader, val_dataloader, METRO_model, smpl, mesh_sampler, renderer):
    ...
    for iteration, (img_keys, images, annotations) in enumerate(train_dataloader): #train_dataloader會將dataset分成好幾小份
        ...
        pred_camera, pred_3d_joints, pred_vertices_sub2, pred_vertices_sub, pred_vertices, joint_domain_pred, vertex_domain_pred = METRO_model(images, smpl, mesh_sampler, meta_masks=meta_masks, is_train=True)
        #-------------------
        ...
        if iteration % args.logging_steps == 0 or iteration == max_iter:
            ...
            logger.info(
                ...
                + '  loss: {:.4f}, 2d joint loss: {:.4f}, 3d joint loss: {:.4f}, vertex loss: {:.4f}, joint domain loss: {:.4f}, vertex domain loss: {:.4f}, compute: {:.4f}, data: {:.4f}, lr: {:.6f}'.format(
                    log_losses.avg, log_loss_2djoints.avg, log_loss_3djoints.avg, log_loss_vertices.avg, log_loss_joint_domain.avg, log_loss_vertex_domain.avg, batch_time.avg, data_time.avg, 
                    optimizer.param_groups[0]['lr'])
            )

            #tensor board
            writer.add_scalar("loss_2d_joints", log_loss_2djoints.avg, iteration /args.logging_steps)
            writer.add_scalar("loss_3d_joints", log_loss_3djoints.avg, iteration /args.logging_steps)
            writer.add_scalar("loss_vertices", log_loss_vertices.avg, iteration /args.logging_steps)
            writer.add_scalar("loss_joint_domain", log_loss_joint_domain.avg, iteration /args.logging_steps)
            writer.add_scalar("loss_vertex_domain", log_loss_vertex_domain.avg, iteration /args.logging_steps)
            writer.add_scalar("total loss", log_losses.avg, iteration /args.logging_steps)
...
def parse_args():
    ...
    #da weight
    parser.add_argument("--vertex_domain_loss_weight", default=10.0, type=float)          
    parser.add_argument("--joint_domain_loss_weight", default=50.0, type=float)
